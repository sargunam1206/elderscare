...
function resetChargeAmounts() {
    ...
    charges.forEach(c => {
        ...
        if (input) {
            input.value = 0;
            input.disabled = false; // re-enable if previously disabled
            // remove Paid badge if it exists
            if (input.nextElementSibling && input.nextElementSibling.classList.contains("badge")) {
                input.nextElementSibling.remove();
            }
            input.style.display = "block"; // show the input again
        }
    });
}
...
function processChargeData(data) {
    ...
    // Set the charge_id from the first record (assuming all have same charge_id)
    if (data && data.length > 0) {
