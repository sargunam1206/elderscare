...
function editServiceBooking(bookingData) {
    ...
    // ✅ CRITICAL: Set service type information from database
    modalEl.setAttribute('data-service-name', bookingData.service_type);
    modalEl.setAttribute('data-service-id', bookingData.service_type_id || '');
        ...
        servicesInfo.forEach((service, index) => {
            ...
            if (newRow) {
                ...
                if (categoryInput && service.item) {
                    ...
                    setTimeout(() => {
                        loadItemsForCategory(newRow, service.item, service.type);
                    }, 100);
                }
...
// Helper function to load items and pre-select the correct one
function loadItemsForCategory(row, category, preSelectedItem = null) {
    ...
    fetch("<?= base_url('service_items') ?>/" + category)
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                ...
                // If we have a pre-selected item, try to find and highlight it
                if (preSelectedItem) {
                    setTimeout(() => {
                        const items = itemList.querySelectorAll('.dropdown-item');
                        items.forEach(itemDiv => {
                            if (itemDiv.getAttribute('data-value') === preSelectedItem) {
                                // You can add some visual indication here if needed
