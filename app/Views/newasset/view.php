...
-->
    ...
    <div class="page-wrapper">
          ...
          <div class="modal fade" id="addAssetModal" tabindex="-1" aria-labelledby="addAssetModalLabel"
            ...
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
              <div class="modal-content">
                <form method="post" id="assetForm" action="<?= base_url('asset1'); ?>" enctype="multipart/form-data">
                       ...
                       <!-- <div class="row gx-2 gy-3"> -->
                        ...
                        <div class="col-md-4 ">
                         ...
                         <div class="mb-3">
                          ...
                          <div class="mb-3">
                            <label for="current_stock" class="form-label required">Current Stock</label>
                            <input type="text" class="form-control" name="current_stock" id="current_stock"
                              placeholder="Enter Current Stock" autocomplete="off" required/>
                          </div>

                        <div class="mb-3">
                            <label for="reorder_level" class="form-label required">Reorder Level</label>
                            <input type="text" class="form-control" name="reorder_level" id="reorder_level"
                              placeholder="Enter the Reorder Level" autocomplete="off" required/>
                          </div>
                  ...
                  <!--        </div>-->
                                            ...
                                            <div class="mb-3">
      <label for="mc_type" class="form-label">M/C Type</label>
      <input type="text" class="form-control" name="mc_type" id="mc_type"
        placeholder="Enter M/C Type" autocomplete="off" />
    </div>


                          <div class="mb-3">
                            <label for="part_name" class="form-label required">Purpose</label>
                            <input type="text" class="form-control" name="purpose" id="purpose"
                              placeholder="Enter the Purpose" autocomplete="off" required/>
                          </div>
                        ...
                        <div class="col-md-4">
                         ...
                         <div class="mb-3">
                          ...
                          <div class="mb-3">
                          <label for="specification" class="form-label required">Specification-Grade</label>
                          <input type="text" class="form-control" name="specification" id="specification"
                            placeholder="Enter the specification" autocomplete="off" required/>
                        </div>
      ...
      <input type="text" class="form-control" name="machine_name" id="machine_name"
        placeholder="Enter Machine Name" autocomplete="off" />
    </div>
                          <div class="mb-3">
                          ...
                          <div class="mb-3">
  <label for="resharpening" class="form-label">Resharpen</label>
  <div class="dropdown">
    <input type="text" class="form-control dropdown-toggle w-100" name="resharpening" id="resharpeningInput"
      placeholder="Select Resharpen" data-bs-toggle="dropdown" aria-expanded="false" autocomplete="off" readonly />
    <ul class="dropdown-menu p-2 w-100" aria-labelledby="resharpeningInput" style="max-height: 150px; overflow-y: auto;">
      <div id="resharpeningLists" style="width: 100%;">
        <div class="dropdown-item" data-value="Yes">Yes</div>
        <div class="dropdown-item" data-value="No">No</div>
      </div>
    </ul>
  </div>
</div>
  ...
  <div id="assetImagePreviewWrapper" style="display: none;">
    <img id="assetImagePreview" src="" alt="Asset Image" class="img-thumbnail mb-2 mt-2" style="max-width: 100px;">
  </div>
</div>
...
<div class="col-12" id="accessories-section">
  <div class="mb-3">
    ...
    <small class="text-muted fw-bold">Manage our inventory of assets</small>
  </div>
...
<!-- Delete Button -->
  <button type="button" class="btn"
   ...
   <i class="bi bi-trash text-danger"></i>
      ...
      <!-- Delete Confirmation Modal -->
<div class="modal fade" id="deleteConfirmationModal" tabindex="-1" aria-labelledby="deleteModalTitle"
  aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header d-flex align-items-center">
        <h5 class="modal-title" id="deleteModalTitle">Are you sure you want to delete this asset?</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-footer d-flex gap-3">
        <a href="#" id="btn-confirm-delete" class="btn btn-danger">Yes</a>
      ...
      <script>
        const resharpeningInput = document.getElementById('resharpeningInput');
const resharpeningItems = document.querySelectorAll('#resharpeningLists .dropdown-item');

resharpeningItems.forEach(item => {
  item.addEventListener('click', function () {
    const value = this.getAttribute('data-value');
    resharpeningInput.value = value;
  });
});
...
function editAsset(asset) {
  ...
  document.getElementById('addAssetModalLabel').textContent = "Edit Assets";

  // Update form action to update URL
  const form = document.getElementById("assetForm");
  form.action = "<?= base_url('updateasset') ?>/" + asset.id;
  ...
  document.getElementById("dealerNameInput").value = asset.dealer_name || '';
  document.getElementById("materialInput").value = asset.material || '';
  document.getElementById("customerNameInput").value = asset.customer_name || '';
  document.getElementById("part_name").value = asset.part_name || '';
  document.getElementById("grn").value = asset.grn || '';
  document.getElementById("current_stock").value = asset.current_stock || '';
  document.getElementById("reorder_level").value = asset.reorder_level || '';
  ...
  //document.getElementById("grade").value = asset.grade || '';
  document.getElementById("uomInput").value = asset.uom || '';
  //document.getElementById("serial_id").value = asset.serial_id || '';
  document.getElementById("calibration_id").value = asset.calibration_id || '';
  document.getElementById("part_number").value = asset.part_number || '';
  document.getElementById("purpose").value = asset.purpose || '';
  //document.getElementById("room").value = asset.room || '';
  document.getElementById("machine_name").value = asset.machine_name || '';
  document.getElementById("mc_type").value = asset.mc_type || '';

  // Resharpening dropdown
  const resharpening = form.querySelector('[name="resharpening"]');
  if (resharpening) {
    resharpening.value = asset.resharpening || '';
  }

  

  // Show image preview only if image exists
  const wrapper = document.getElementById('assetImagePreviewWrapper');
  const img = document.getElementById('assetImagePreview');

  if (asset.asset_image) {
    img.src = `<?= base_url() ?>/${asset.asset_image}`;
  ...
  if (Array.isArray(accessories)) {
    accessories.forEach(item => {
      ...
      <div class="col-md-3">
       ...
       <div class="col-md-2">
        <label>Specification</label>
        <input type="text" class="form-control accessory-specification" placeholder=""  value="${item.type}"required>
      </div>
      <div class="col-md-2">
        <label>Stock</label>
        <input type="number" class="form-control accessory-qty" placeholder="" value="${item.quantity}" required>
      </div>
...
document.getElementById('addAssetModal').addEventListener('hidden.bs.modal', function () {
  ...
  form.action = "<?= base_url('asset1'); ?>"; // Reset to "Add" mode
  document.getElementById('assetImagePreview').src = '';
  document.getElementById('addAssetModalLabel').textContent = "Add Assets";
   // Hide image preview in Add mode
  document.getElementById('assetImagePreviewWrapper').style.display = 'none';
});
</script>

<script>
  function setDeleteUrl(assetId) {
    const deleteBtn = document.getElementById('btn-confirm-delete');
    deleteBtn.href = "<?= base_url('deleteasset') ?>/" + assetId;
  }
</script>
