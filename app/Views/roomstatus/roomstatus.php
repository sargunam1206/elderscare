...
fetch('<?= base_url('getOccupiedRooms') ?>')
    .then(res => res.json())
    .then(data => {
        occupiedBookings = data.occupiedBookings || [];
        console.log("Occupied Bookings:", occupiedBookings);
    })
    .catch(err => console.error("Error fetching occupied rooms:", err));
</script>

<script> 
const servicetype = <?= json_encode($servicetype) ?>;
...
menu.querySelector(".pos-btn").addEventListener("click", (e) => {
    e.stopPropagation();
    handleMenuClick(() => {
        // Filter bookings for the clicked room
        const bookingsForRoom = occupiedBookings.filter(b => b.room_id == roomId);

        let rowsHtml = '';
        if (bookingsForRoom.length > 0) {
            bookingsForRoom.forEach(b => {
                if (b.guest1_firstname) {
                    rowsHtml += `
                        <tr data-guest-id="${b.guest1_id}" data-guest-type="guest1" data-room-no="${b.room}" >
                          <td>${(b.guest1_firstname || '') + ' ' + (b.guest1_lastname || '')}</td>
                          <td>${b.room}</td>
                          <td>
                            <div class="dropdown">
                              <input type="text" 
  ...
  : `<div class="dropdown-item text-muted">No services found</div>`
          ...
          <button class="btn btn-primary maintenance-pay-btn"> Pay Maintenance Charges
    </button>
                          </td>
                        </tr>`;
                }
                if (b.guest2_firstname) {
                    rowsHtml += `
                        <tr data-guest-id="${b.guest2_id}" data-guest-type="guest2" data-room-no="${b.room}">
                          <td>${(b.guest2_firstname || '') + ' ' + (b.guest2_lastname || '')}</td>
                          <td>${b.room}</td>
                           <td>
                            <div class="dropdown">
                              <input type="text" 
...
${servicetype.length > 0 ? 
  ...
  : `<div class="dropdown-item text-muted">No services found</div>`
           ...
           <button class="btn btn-primary maintenance-pay-btn"> Pay Maintenance Charges
            ...
            rowsHtml = `<tr><td colspan="4" class="text-center">No guests found for this room</td></tr>`;
        }
        ...
        document.getElementById("room-info").innerHTML = `
            <div class="pos-section">
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-striped w-100 table-bordered text-nowrap align-middle">
                        <thead class="table-light">
                            <tr>
                                <th>Guest Name</th>
                                <th>Room No</th>
...
// RE-ATTACH event listeners to the new maintenance buttons
setTimeout(() => {
    document.querySelectorAll('[data-bs-target="#maintenanceModal"]').forEach(button => {
...
function openMaintenanceModal(button) {
 ...
 console.log("Button clicked:", button);
    ...
    console.log("Data attributes:", {
        guestId: row.getAttribute('data-guest-id'),
        guestType: row.getAttribute('data-guest-type'), 
        roomNo: row.getAttribute('data-room-no')
    });
    ...
    currentRoomNo = row.getAttribute('data-room-no');
     ...
     console.log("Values:", { currentGuestId, currentGuestType, currentRoomNo });
...
// Add event listener for the new class
document.addEventListener('click', function(e) {
    if (e.target.closest('.maintenance-pay-btn')) {
        e.preventDefault();
        openMaintenanceModal(e.target.closest('.maintenance-pay-btn'));
