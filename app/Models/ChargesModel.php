...
public function getFilteredCharges($filters = [])
{
    $builder = $this->db->table('charges c');
    $builder->select('c.*, r.room_no as room_number, 
        CASE 
            ...
            ELSE g.first_name 
        END as guest_name');
    $builder->join('rooms r', 'c.room_no = r.room_no', 'left');
    $builder->join('guests_personal g', 'c.guest_id = g.guest_id', 'left');
    $builder->where('c.deleted_at', null);
    
    // Apply filters
    if (!empty($filters['from_date'])) {
        $builder->where('c.due_date >=', $filters['from_date']);
    }
    
    if (!empty($filters['to_date'])) {
        $builder->where('c.due_date <=', $filters['to_date']);
    }
    ...
    if (!empty($filters['room']) && $filters['room'] !== 'all') {
        $builder->where('c.room_no', $filters['room']);
    }
    
    $builder->orderBy('c.due_date', 'DESC');
    
    $query = $builder->get();
    $charges = $query->getResultArray();
