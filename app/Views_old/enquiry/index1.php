        ...
        <div class="col-md-12">
          <div class="card p-4 h-100 d-flex flex-column">
            <div class="d-flex justify-content-between align-items-center mb-3">
              <h5><i class="bi bi-tools me-2 text-success"></i>Enquiry</h5>
              <div>
            ...
            <div class="modal-body">
              ...
              <div class="row mb-3">
                ...
                <div class="col-md-6">
                  ...
                  <input type="number" class="form-control" name="guest_count" id="guest_count" min="1" placeholder="Number of guests" required>
  ...
  function editRequest(id) {
    ...
    fetch(`${baseUrl}enquiry/edit/${id}`)
        ...
        .then(data => {
            ...
            // Update form action
            document.getElementById('enquiryForm').action = `${baseUrl}maintenance/update/${data.id}`;
            
            // Update UI
            document.getElementById('enquiryModalLabel').innerHTML = '<i class="bi bi-pencil-square me-2"></i>Edit Maintenance Request';
...
['problemFileList', 'billFileList', 'clearanceFileList'].forEach(id => {
    ...
    if (fileList) fileList.innerHTML = '';
            ...
            data.problem_photos.forEach(file => {
                ...
                fileItem.innerHTML = `
                    <a href="javascript:void(0)" class="file-preview-link"
                        onclick="previewMaintenanceFile('${file.file_path}', '${file.original_name}')">
                        <i class="bi ${getFileIcon(file.original_name)} me-1"></i>
                        ${file.original_name.substring(0, 15)}${file.original_name.length > 15 ? '...' : ''}
                    </a>
        ...
        if (data.payment_bill) {
            ...
            fileItem.innerHTML = `
                <a href="javascript:void(0)" class="file-preview-link"
                    onclick="previewMaintenanceFile('${data.payment_bill.file_path}', '${data.payment_bill.original_name}')">
                    <i class="bi ${getFileIcon(data.payment_bill.original_name)} me-1"></i>
                    ${data.payment_bill.original_name.substring(0, 15)}${data.payment_bill.original_name.length > 15 ? '...' : ''}
                </a>
                <small class="text-muted ms-2">(Current file)</small>
            `;
            container.appendChild(fileItem);
