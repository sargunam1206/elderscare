...
class Asset extends BaseController
{
    ...
    function __construct()
      ...
      public function index()
      {
          ...
          if ($this->request->getPost('submit')) {
              ...
              // Add the GRN to the asset data
              $full['grn'] = $generatedGrn;
      
              // Save the data to the asset table
              if ($this->assetModel->save($full)) { // Save asset and check success
                  ...
                  if ($assetId) {
                      ...
                      $this->session->setFlashdata('success', 'Asset Added successfully.');
                  }
              } else {
          ...
          public function all_purposes()
          {
              ...
              $purposes = $assetModel
                  ->select('purpose')
                  ->where('purpose IS NOT NULL') // Exclude null purposes
    ...
    public function all_dealers()
    {
        ...
        $dealers = $dealerModel->select('dealer_name')->groupBy('dealer_name')->where('deleted_on', NULL)->findAll();
...
public function all_part_numbers()
{
    ...
    $partnumbers = $assetModel->select('part_number')->groupBy('part_number')->where('deleted_on', NULL)->findAll();
    ...
    public function all_customers()
    {
      ...
      // Fetch  from the database
      $customers = $assetModel->select('customer_name')->groupBy('customer_name')->where('deleted_on', NULL)->findAll();
    ...
    public function all_materials()
    {
      ...
      // Fetch  from the database
      $materials = $assetModel->select('material')->groupBy('material')->where('deleted_on', NULL)->findAll();
 ...
 public function all_locations()
  {
      ...
      $locations = $assetModel->select('location')->groupBy('location')->where('deleted_on', NULL)->findAll();
  ...
  public function all_rooms()
  {
      ...
      // Fetch  from the database
      $rooms = $assetModel->select('room')->groupBy('room')->where('deleted_on', NULL)->findAll();
      ...
      public function edit($id)
      {
          ...
          // Pass the asset data with the correct variable name
          return view('newasset/edit', ['asset' => $asset]); 
      }
    ...
    public function update($id)
    {
        if ($this->request->getPost('submit')) {
            ...
            $this->session->setFlashdata('success', 'Asset updated successfully.');
    ...
    public function delete($id)
    {
        ...
        if ($this->assetModel->update($id, $updateData)) {
            // Set a success message
            $this->session->setFlashdata('success', 'Asset deleted successfully.');
        } else {
    ...
    public function delete_assets()
    {
        ...
        if (isset($input->ids) && is_array($input->ids)) {
            ...
            if ($successCount > 0) {
                $this->session->setFlashdata('success', "$successCount asset(s) deleted successfully.");
            }
            if ($errorCount > 0) {
                $this->session->setFlashdata('error', "$errorCount Asset(s) failed to be deleted. Please try again.");
            }
    ...
    $assets = $assetModel->where('deleted_on', NULL)->findAll(); // Apply filters if needed
