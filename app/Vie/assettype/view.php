...
<!--
                <li class="sidebar-item ">
                  <a class="sidebar-link has-arrow" href="<//?= base_url('assetsvalue'); ?>" aria-expanded="false">
                  
                    <span class="hide-menu">Assets Value</span>
                  </a>
                   <ul aria-expanded="false" class="collapse first-level">
                      <li class="sidebar-item">
                        <a class="sidebar-link" href="<//?= base_url('assetsvalue'); ?>">
                          <span class="icon-small"></span>Add Assetsvalue
                        </a>
                      </li>
                      <li class="sidebar-item">
                        <a class="sidebar-link" href="<//?= base_url('viewassetsvalue'); ?>">
                          <span class="icon-small"></span>View Assetsvalue
                        </a>
                      </li>
            ...
            <div class="row align-items-center">
              <div class="col-12">
                <div class="d-sm-flex align-items-center justify-space-between">
                  <h4 class="mb-4 mb-sm-0 card-title">Admin</h4>
...
<div class="modal fade" id="vertical-center-scroll-modal" tabindex="-1" aria-labelledby="vertical-center-modal" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('assettype'); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title" id="myLargeModalLabel">Add Asset Type</h4>
        ...
        <div class="modal-body">
          ...
          <div class="row">
            <div class="col-12">
              <div class="mb-3">
                <label for="asset_type" class="form-label required">Asset Type Name</label>
                <div class="dropdown">
                  <input type="text" class="form-control dropdown-toggle w-100" name="asset_type" id="assetTypeInput" placeholder="Enter Asset Type" data-bs-toggle="dropdown" aria-expanded="false" autocomplete="off" required />
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="col text-end">
    <div class="form-check d-inline-block">
      ...
      <label for="selectAll" class="form-check-label">Select All</label>
          ...
          <div class="row">
  <?php
    $chunks = array_chunk($input_field, ceil(count($input_field) / 3)); // Split into 3 columns
      ...
      <?php foreach ($column as $label): ?>
        <div class="form-check mb-2">
          <input
            ...
            <?= trim($label) === 'Asset Type' ? 'checked' : '' ?>
            <?= trim($label) === 'Asset Make' ? 'checked' : '' ?>
            <?= trim($label) === 'Specification' ? 'checked' : '' ?>
            <?= trim($label) === 'GRN' ? 'checked' : '' ?>
            <?= trim($label) === 'Current Stock' ? 'checked' : '' ?>
          >
...
<!-- <button type="button" class="btn mb-1 bg-danger-subtle text-danger px-4 fs-4 " data-bs-toggle="modal" data-bs-target="#vertical-center-scroll-modal"> -->
  ...
  <table id="form_inputs_assettype" class="table table-striped w-100 table-bordered display text-nowrap align-middle">
  <thead>
        <tr>
        <!-- <th><input type="checkbox" id="select_all"></th>  -->
           <th style="width: 50px;">ID</th>
    <th style="width: 150px;">Asset Type</th> <!-- Decreased width -->
    <th style="width: 60%;">Form Inputs</th>     <!-- Increased width -->
    <th style="width: 130px;">Actions</th>      <!-- Decreased width -->
        </tr>
    </thead>
    <tbody>
  
        <?php $i = 1; foreach ($assettypes as $asset):
     ...
     <td>
  <?php
    $fields = json_decode($asset['asset_form_field'], true);
    if (is_array($fields)) {
        $chunks = array_chunk($fields, 9);
        foreach ($chunks as $chunk) {
            echo '<div class="mb-1 d-flex flex-wrap gap-1">';
            foreach ($chunk as $field) {
                echo '<span class="badge bg-primary text-white">' . htmlspecialchars($field) . '</span>';
            }
            echo '</div>';
        }
    }
...
<button type="button"
 ...
 <!-- Edit Modal for asset ID: <?= $asset['id']; ?> -->
<!-- Edit Modal -->
<div class="modal fade" id="editModal<?= $asset['id']; ?>" tabindex="-1" aria-labelledby="editModalLabel<?= $asset['id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('updateassettype/' . $asset['id']); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title " id="editModalLabel<?= $asset['id']; ?>">Edit Asset Type</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        ...
        <div class="modal-body">
              ...
              <div class="mb-3">
                ...
                <div class="dropdown">
                  <input
                    ...
                    aria-expanded="false"
                    autocomplete="off"
                    value="<?= htmlspecialchars($asset['asset_type']); ?>"
                    required
                  />
                </div>
              </div>
          ...
          <div class="row align-items-center mb-2">
            ...
            <div class="col text-end">
              <div class="form-check d-inline-block">
                <input type="checkbox" id="selectAll<?= $asset['id']; ?>" class="form-check-input"
                       onclick="toggleEditAll(this, 'editModal<?= $asset['id']; ?>')">
                <label for="selectAll<?= $asset['id']; ?>" class="form-check-label">Select All</label>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <?php
              $selected_fields = json_decode($asset['asset_form_field'], true) ?? [];
              $chunks = array_chunk($input_field, ceil(count($input_field) / 3));
              foreach ($chunks as $column):
            ?>
              <div class="col-md-4">
                <?php foreach ($column as $label): ?>
                  <div class="form-check mb-2">
                    <input
                      ...
                      name="fields[]"
                      value="<?= htmlspecialchars($label) ?>"
                      <?= in_array($label, $selected_fields) ? 'checked' : '' ?>
                    >
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer d-flex gap-3 justify-content-end">
        ...
        <a href="<?= base_url('deleteassettype/' . $asset['id']); ?>" class="btn btn-danger">Yes</a>
...
?>
 ...
 <?php if ($activeTab === 'make' && $successMessage): ?>
    <div class="alert bg-primary-subtle text-info alert-dismissible fade show" role="alert">
        <div class="d-flex align-items-center text-primary">
            <i class="ti ti-info-circle me-2 fs-4"></i>
            <?= $successMessage ?>
...
<?php endif; ?>                 
                  ...
                  <form method="post"  action="<?= base_url('viewassetmake'); ?>">
                           <div class="text-end mb-3">
  <!-- <a href="<//?= base_url('assettype'); ?>" class="btn btn-primary">Add Asset Type</a> -->
   <button type="button" 
   class="btn btn-primary" 
   data-bs-toggle="modal" 
   data-bs-target="#assetMakeModal">
   + Add Asset Make
</button>
<!-- <button type="button" class="btn mb-1 bg-danger-subtle text-danger px-4 fs-4 " data-bs-toggle="modal" data-bs-target="#vertical-center-scroll-modal"> -->
                  ...
                  <!--a href="<//?= base_url('/assettype/create'); ?>" class="btn btn-primary">Add New</a>-->
  ...
  <table id="form_inputs_assetmake" class="table table-striped w-100 table-bordered display text-nowrap align-middle">
  <thead>
        <tr>
        <!-- <th><input type="checkbox" id="select_all"></th> -->
         <!-- Select all checkbox -->
    ...
    <tbody>
        <?php $i = 1; foreach ($assetmakes as $asset): ?>
            <tr>
            <!-- <td><input type="checkbox" class="row_checkbox" data-asset-id="<?= $asset['id']; ?>"></td> -->
                <!--<td><?= $asset['id']; ?></td>-->
                <td><?= $i++; ?></td>
...
<button type="button"
        class="btn" style="color:blue"
        data-bs-toggle="modal"
        data-bs-target="#editAssetMakeModal<?= $asset['id']; ?>">
  <i class="bi bi-pencil-square" ></i>
</button>
...
<a href="javascript:void(0)" 
   class="btn" 
   data-bs-toggle="modal" 
   data-bs-target="#deleteAssetMakeModal<?= $asset['id']; ?>">
  <i class="bi bi-trash text-danger"></i>
</a>



                <!-- <a href="<//?= base_url('editassetmake/'. $asset['id']); ?>" class="btn btn-info">Update</a> -->
        ...
        <?php foreach ($assetmakes as $asset): ?>
 <!-- Edit Modal -->
<div class="modal fade" id="editAssetMakeModal<?= $asset['id']; ?>" tabindex="-1" aria-labelledby="editAssetMakeLabel<?= $asset['id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('updateassetmake/' . $asset['id']); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title" id="editAssetMakeLabel<?= $asset['id']; ?>">Edit Asset Make</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
            ...
            <div class="col-12">
              <div class="mb-3">
                ...
                <input type="text" class="form-control w-100"
                       ...
                       autocomplete="off"
                       value="<?= htmlspecialchars($asset['asset_make']); ?>"
                       required />
              </div>
            </div>
          </div>
...
<!-- Delete Confirmation Modal -->
<div class="modal fade" id="deleteAssetMakeModal<?= $asset['id']; ?>" tabindex="-1" aria-labelledby="deleteAssetMakeLabel<?= $asset['id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header d-flex align-items-center">
        <h5 class="modal-title" id="deleteAssetMakeLabel<?= $asset['id']; ?>">Are you sure you want to delete?</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-footer justify-content-end">
        <a href="<?= base_url('deleteassetmake/' . $asset['id']); ?>" class="btn btn-danger">Yes</a>
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="assetMakeModal" tabindex="-1" aria-labelledby="assetMakeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('assetmake'); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title" id="assetMakeModalLabel">Add Asset Make</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
...
?>
 ...
 <?php if ($activeTab === 'dealers' && $successMessage): ?>
    <div class="alert bg-primary-subtle text-info alert-dismissible fade show" role="alert">
        <div class="d-flex align-items-center text-primary">
            <i class="ti ti-info-circle me-2 fs-4"></i>
            <?= $successMessage ?>
...
<?php endif; ?> 
                ...
                <form method="post" action="<?= base_url('viewdealers'); ?>">
                    <div class="text-end mb-3">
                      <!-- Button to trigger Dealer Modal -->
<button type="button" 
        class="btn btn-primary" 
        data-bs-toggle="modal" 
        data-bs-target="#dealerModal">
  + Add Dealer Name
</button>
                    </div>
                  <!--a href="<//?= base_url('/assettype/create'); ?>" class="btn btn-primary">Add New</a>-->
                  ...
                  <td class="p-1">
                    <!-- <a href="javascript:void(0)" id="btn-delete-trigger" class="btn btn-danger "><i
                  ...
                  <div class="table-responsive mt-3">
                    ...
                    <tr>
                      ...
                      <table id="form_inputs"
                        ...
                        <tbody>
                          <?php $i = 1;foreach ($dealer as $asset): ?>
                            <tr>
                              <!-- <td><input type="checkbox" class="row_checkbox" data-asset-id="<?= $asset['id']; ?>"></td> -->
                              <!--<td><?= $asset['id']; ?></td>-->
                              <td><?= $i++; ?></td>
...
<button type="button"
        class="btn" style="color:blue"
        data-bs-toggle="modal"
        data-bs-target="#editDealerModal<?= $asset['id']; ?>">
   <i class="bi bi-pencil-square" ></i>
</button>
...
<a href="javascript:void(0)" 
   class="btn" 
   data-bs-toggle="modal" 
   data-bs-target="#deleteDealerModal<?= $asset['id']; ?>">
  <i class="bi bi-trash text-danger"></i>
</a>
                                ...
                                <!-- <a href="<//?= base_url('editdealers/' . $asset['id']); ?>" class="btn btn-info">Update</a> -->
                  ...
                  <?php foreach ($dealer as $asset): ?>
 <!-- Edit Dealer Modal -->
<div class="modal fade" id="editDealerModal<?= $asset['id']; ?>" tabindex="-1" aria-labelledby="editDealerLabel<?= $asset['id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('updatedealers/' . $asset['id']); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title" id="editDealerLabel<?= $asset['id']; ?>">Edit Dealer Name</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        ...
        <div class="modal-body">
          <div class="row">
            <div class="col-12">
              <div class="mb-3">
                <label for="dealer_name" class="form-label required">Dealer Name</label>
                <input type="text"
                       class="form-control"
                       name="dealer_name"
                       placeholder="Enter Dealer Name"
                       value="<?= htmlspecialchars($asset['dealer_name']); ?>"
                       autocomplete="off" 
                       required/>
              </div>
            </div>
...
<div class="modal fade" id="deleteDealerModal<?= $asset['id']; ?>" tabindex="-1" aria-labelledby="deleteDealerLabel<?= $asset['id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header d-flex align-items-center">
        <h5 class="modal-title" id="deleteDealerLabel<?= $asset['id']; ?>">Are you sure you want to delete this dealer?</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-footer justify-content-end">
        <a href="<?= base_url('deletedealers/' . $asset['id']); ?>" class="btn btn-danger">Yes</a>
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="dealerModal" tabindex="-1" aria-labelledby="dealerModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('dealers'); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title" id="dealerModalLabel">Add Dealer Name</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
...
?>
 ...
 <?php if ($activeTab === 'uom' && $successMessage): ?>
    <div class="alert bg-primary-subtle text-info alert-dismissible fade show" role="alert">
        <div class="d-flex align-items-center text-primary">
            <i class="ti ti-info-circle me-2 fs-4"></i>
            <?= $successMessage ?>
...
<button type="button"
       class="btn" style="color:blue"
        data-bs-toggle="modal"
        data-bs-target="#editUOMModal<?= $asset['id']; ?>">
   <i class="bi bi-pencil-square" ></i>
</button>
...
<a href="javascript:void(0)"
   class="btn"
   data-bs-toggle="modal"
   data-bs-target="#deleteUOMModal<?= $asset['id']; ?>">
 <i class="bi bi-trash text-danger"></i>
</a>
                                ...
                                <!-- <a href="<//?= base_url('edituom/' . $asset['id']); ?>" class="btn btn-info">Update</a> -->
                  ...
                  <?php foreach ($uom as $asset): ?>
  <!-- Edit Modal -->
 <div class="modal fade" id="editUOMModal<?= $asset['id']; ?>" tabindex="-1" aria-labelledby="editUOMLabel<?= $asset['id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('updateuom/' . $asset['id']); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title" id="editUOMLabel<?= $asset['id']; ?>">Edit Unit of Measurement (UOM)</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
            ...
            <div class="col-12">
              <div class="mb-3">
                ...
                <input type="text"
                       ...
                       value="<?= htmlspecialchars($asset['uom']); ?>"
                       autocomplete="off"
                       required />
              </div>
            </div>
...
<div class="modal fade" id="deleteUOMModal<?= $asset['id']; ?>" tabindex="-1" aria-labelledby="deleteUOMLabel<?= $asset['id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header d-flex align-items-center">
        <h5 class="modal-title" id="deleteUOMLabel<?= $asset['id']; ?>">Are you sure you want to delete this UOM?</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-footer justify-content-end">
        <a href="<?= base_url('deleteuom/' . $asset['id']); ?>" class="btn btn-danger">Yes</a>
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="uomModal" tabindex="-1" aria-labelledby="uomModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post" action="<?= base_url('uom'); ?>" enctype="multipart/form-data">
        <div class="modal-header d-flex align-items-center">
          <h4 class="modal-title" id="uomModalLabel">Add Unit of Measurement (UOM)</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
              ...
              <div class="mb-3">
                ...
                <div class="dropdown">
                  <input type="text" 
                         ...
                         name="uom" 
                         id="uomInput" 
                         placeholder="Enter Unit of Measurement (UOM)" 
                         data-bs-toggle="dropdown" 
                         aria-expanded="false" 
                         autocomplete="off" 
                         required/>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (tab === 'make') {
      const makeTab = document.getElementById('pills-notifications-tab');
      if (makeTab) new bootstrap.Tab(makeTab).show();
    }

    if (tab === 'dealers') {
      const dealersTab = document.getElementById('pills-bills-tab');
      if (dealersTab) new bootstrap.Tab(dealersTab).show();
    }
    if (tab === 'uom') {
      const uomTab = document.getElementById('pills-security-tab');
      if (uomTab) new bootstrap.Tab(uomTab).show();
    }
  });
</script>
