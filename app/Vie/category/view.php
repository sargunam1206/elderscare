...
<div class="modal fade" id="vertical-center-scroll-modal" tabindex="-1" aria-labelledby="vertical-center-modal" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      <form method="post"  id="assetForm"  action="<?= base_url('addcategory'); ?>" enctype="multipart/form-data">
     ...
     <div class="col-12">
      <div class="mb-3">
          <label for="room_no" class="form-label required">Category Name</label>
        <div class="dropdown">
                    <input type="text" class="form-control dropdown-toggle w-100" name="category_name" id="category_name" placeholder="Enter Category Name "  autocomplete="off" />
...
<div class="col-12" id="part_number" >
    <div class="mb-3">
  <label for="relationInput" class="form-label required">Service Type</label>
  <div class="dropdown">
    <input type="text" class="form-control dropdown-toggle w-100" name="service_type_name" id="service_type_name"
      placeholder="Select Relation" data-bs-toggle="dropdown" aria-expanded="false" autocomplete="off" readonly />
    <ul class="dropdown-menu p-2 w-100" aria-labelledby="relationInput" style="max-height: 150px; overflow-y: auto;">
      <div id="relationLists" style="width: 100%;">
        ...
        <div class="dropdown-item" data-value="<?= $type['name'] ?>"><?= $type['name'] ?></div>
   ...
   <div class="col-12">
      <div class="mb-3">
          <label for="room_no" class="form-label required">Short Code</label>
        <div class="dropdown">
                    <input type="text" class="form-control dropdown-toggle w-100" name="short_code" id="short_code" placeholder="Enter Short Code "  autocomplete="off" />
...
Service Category</h5>
  ...
  <thead>
        ...
        <!-- <th><input type="checkbox" id="select_all"></th>  -->
           <th>S.No</th>
    <th>Service Type</th> <!-- Decreased width -->
   ...
   <th >Short Code</th> 
    ...
    <tbody>
        ...
        <?php $i = 1; foreach ($category as $asset):
          // $base=base64_encode(base64_encode(base64_encode($asset['id'])));
            ...
            <tr>
                ...
                <td><?= $i++; ?> </td>
                <td><?= $asset['service_type_name']; ?></td>
                <td><?= $asset['category_name']; ?></td>
                <td><?= $asset['short_code']; ?></td>
...
<button type="button"
 ...
 <a href="javascript:void(0)" 
   class="btn " 
   data-bs-toggle="modal" 
   data-bs-target="#deleteConfirmationModal<?= $asset['category_id']; ?>">
 <i class="bi bi-trash text-danger"></i>
            ...
            <div class="modal fade" id="deleteConfirmationModal<?= $asset['category_id']; ?>" tabindex="-1" aria-labelledby="deleteModalTitle<?= $asset['category_id']; ?>" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header d-flex align-items-center">
        <h5 class="modal-title" id="deleteModalTitle<?= $asset['category_id']; ?>">Are you sure you want to delete?</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-footer d-flex gap-3 justify-content-end">
        <!-- Confirm Delete Button -->
        <a href="<?= base_url('deletecategory/' . $asset['category_id']); ?>" class="btn btn-danger">Yes</a>
  ...
  <script>
  const relationInput = document.getElementById('service_type_name');
...
function editAsset(asset) {
  ...
  document.getElementById('myLargeModalLabel').textContent = "Edit Category";

  // Update form action to update URL
  const form = document.getElementById("assetForm");
  form.action = "<?= base_url('updatecategory') ?>/" + asset.category_id ;
  
  // Set all input values
  document.getElementById("service_type_name").value = asset.service_type_name || '';

  document.getElementById('category_name').value = asset.category_name || '';
  document.getElementById('short_code').value = asset.short_code || '';
...
document.getElementById('vertical-center-scroll-modal').addEventListener('hidden.bs.modal', function () {
    document.getElementById('myLargeModalLabel').textContent = "Add Category";
  const form = document.getElementById('assetForm');
  form.reset();
  document.getElementById('addaccessory-container').innerHTML = '';
  form.action = "<?= base_url('category'); ?>"; // Reset to "Add" mode
