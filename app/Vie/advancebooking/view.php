...
<!-- <script>
    ...
    <!-- <div id="reservationForm"> -->
      ...
      <!-- <div class="header mt-3 mb-0">View Advance Booking</div> -->
       ...
       <div class="datatables">
              ...
              <h5 class="fs-5"><i class="bi bi-list-check me-2 text-success"></i>Booking List</h5>
              <div>
  ...
  <div class="dropdown">
    <input type="text" class="form-control dropdown-toggle w-100" name="typeDisplay" id="typeFilterDisplay"
      placeholder="Select Room Type" data-bs-toggle="dropdown" aria-expanded="false" autocomplete="off" readonly value="All" />
    ...
    <ul class="dropdown-menu p-2 w-100" aria-labelledby="typeFilterDisplay" style="max-height: 150px; overflow-y: auto;">
      <div id="roomTypeLists" style="width: 100%;">
  ...
  <div class="dropdown">
    <input type="text" class="form-control dropdown-toggle w-100" name="statusDisplay" id="statusFilterDisplay"
      placeholder="Select Status" data-bs-toggle="dropdown" aria-expanded="false" autocomplete="off" readonly value="All" />
    <!-- Hidden input to store the actual value sent to backend -->
    ...
    <!-- PDF Button -->
    <div class="col-md-1">
      <!-- <button type="button" id="pdfButton" class="btn btn-primary w-100">PDF</button> -->
       <button type="button" id="pdfButton" class="btn btn-success w-100">PDF</button>

    </div>
    <!-- Excel Button -->
    <div class="col-md-1">
       <button type="submit" formaction="<?= base_url('advancebooking/export') ?>" class="btn btn-primary w-100">Excel</button>
    </div>

    <div class="col-md-1">
      <!-- <button type="button" id="resetBtn" class="btn btn-primary w-100">Reset</button> -->
                          ...
                          <table id="form_inputs"
                            ...
                            <thead>
                              <tr>
                                ...
                                <th style="width: 150px;">Type</th> <!-- Increased width -->
                                <th style="width: 150px;">Room No</th>
                                <th style="width: 150px;">Status</th>
                                <th style="width: 150px;">Date of Booking</th>
                                <th style="width: 150px;">Guest Info</th>
...
<td>  <?= date("M d, Y", strtotime($booking['created_on'])); ?></td>

<td>                                                         <!-- PDF icon for Guest 1 -->
<?php if (!empty($booking['guest1_id'])): ?>
<a href="javascript:void(0)" class="btn generate-pdf" 
   data-guest-id="<?= $booking['guest1_id']; ?>"
   title="Generate PDF for Guest 1">
   <i class="bi bi-person text-success">1</i>
    <!-- <i class="bi bi-file-earmark-pdf text-danger"></i>1 -->
</a>
<?php endif; ?>

<!-- PDF icon for Guest 2 -->
<?php if (!empty($booking['guest2_id'])): ?>
<a href="javascript:void(0)" class="btn generate-pdf" 
   data-guest-id="<?= $booking['guest2_id']; ?>"
   title="Generate PDF for Guest 2">
    <i class="bi bi-person text-success">2</i>
    <!-- <i class="bi bi-file-earmark-pdf text-danger"></i>2 -->
</a>
<?php endif; ?>
</td>
...
<!-- Validation Modal with Edit Button -->
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="missingFieldsList" class="mt-3"></div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary" id="goToEditButton">Go to Edit Page</button>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('change-status')) {
            ...
            // For check-in status, perform validation first
            if (currentNewStatus === 'checked_in') {
                validateBookingBeforeCheckIn(currentBookingId)
                    .then(validationResult => {
                        if (validationResult.isValid) {
                            showConfirmationModal(requireReason);
                        } else {
                            showValidationErrors(validationResult.message, validationResult.missingFields, currentBookingId);
                        }
                    })
                    .catch(error => {
                        console.error('Validation error:', error);
                        showValidationErrors('Error validating booking for check-in', [], currentBookingId);
                    });
            } else {
                // For other status changes, show confirmation directly
                showConfirmationModal(requireReason);
            }
        }
    });
    
    function showConfirmationModal(requireReason) {
    ...
    // Handle Go to Edit button
    document.getElementById('goToEditButton').addEventListener('click', function() {
        if (currentBookingId) {
            window.location.href = `<?= base_url('editadvancebooking') ?>/${currentBookingId}?from=checked_in`;
        }
    ...
    document.querySelectorAll('[data-bs-dismiss="modal"]').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (modalId === 'confirmStatusModal') {
                confirmModal.hide();
            } else if (modalId === 'validationModal') {
                validationModal.hide();
            }
        });
    });
...
async function validateBookingBeforeCheckIn(bookingId) {
    try {
        const response = await fetch(`<?= base_url('validatebooking') ?>/${bookingId}?new_status=checked_in`);
        if (!response.ok) {
        ...
        return {
            ...
            message: data.message || 'Cannot proceed with check-in due to missing information',
        ...
        return {
            isValid: false,
            message: 'Error validating booking for check-in',
            missingFields: ['System error during validation']
        };
    }
}

function showValidationErrors(message, missingFields, bookingId) {
    const validationModal = new bootstrap.Modal(document.getElementById('validationModal'));
    ...
    if (missingFields && missingFields.length > 0) {
        ...
        listTitle.textContent = 'Missing/Invalid Fields:';
        fieldsList.appendChild(listTitle);
...
<!-- jQuery (must load before DataTables or custom scripts) -->
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

<!-- DataTables -->
<script src="<?= base_url(); ?>/public/dist/assets/libs/datatables.net/js/jquery.dataTables.min.js"></script>
<script src="<?= base_url(); ?>/public/dist/assets/libs/datatables.net-bs5/js/dataTables.bootstrap5.min.js"></script>
