...
class Prescription extends BaseController
{
    ...
    public function index($id='')
    {
        ...
        $prescriptions = $this->prescriptionModel
            ->select('prescriptions.*,guests_personal.*')
             ->join('guests_personal', 'guests_personal.guest_id = prescriptions.guest_id')
            ->where('prescriptions.deleted_on', null)
            ->orderBy('prescriptions.created_on', 'DESC')
            ->findAll();
        ...
        $data['guest'] = $this->guestinfo
         ...
         $prescriptions = $this->prescriptionModel
           ->select('prescriptions.*,guests_personal.*')
            ...
            ->where('prescriptions.guest_id', $id)
            ->where('prescriptions.deleted_on', null)
            ->orderBy('prescriptions.created_on', 'DESC')
            ->findAll();
...
public function store()
{
    ...
    try {
      ...
      if($this->request->getPost('type_user')=="single"){
         ...
         return redirect()->to("/prescri/".$guest_id)->with('message', 'Prescription added successfully!');
      }else{
