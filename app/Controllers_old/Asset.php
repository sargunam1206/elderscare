...
public function index()
{
    ...
    if ($this->assetModel->save($full)) { // Save asset and check success
        ...
        //return redirect()->to(current_url());
        return redirect()->to(base_url('viewasset'));
    }
    ...
    return view('newasset/index', ['grn' => $generatedGrn]);
          ...
          helper('grn');
    $grnModel = new \App\Models\GrnModel();
    $generatedGrn = generateGrn($grnModel);
            ...
            if (!empty($partnumberFilters)) {
                $query->whereIn('part_name', $partnumberFilters); // Filter by selected partnumber
            }
    ...
    $data['grn'] = $generatedGrn;
          ...
          //return view('newasset/view', $data);
            ...
            //   $asset['accessories'] = json_decode(json_decode($asset['accessories'], true), true);
            foreach ($data['assets'] as &$asset) {
    if (!empty($asset['accessories'])) {
        $decoded = json_decode($asset['accessories'], true);
        $asset['accessories'] = is_string($decoded) ? json_decode($decoded, true) : $decoded;
    }
}
...
public function all_part_name()
{
    // Load the model
    $assetModel = new AssetModel();
    
    // Fetch part numbers from the database
    $partnumbers = $assetModel->select('part_name')->groupBy('part_name')->where('part_name!=', '')->where('deleted_on', NULL)->findAll();
    
    // Return part numbers as JSON response
    return $this->response->setJSON($partnumbers);
}

public function all_grn_no()
{
     ini_set('display_errors', '1');
          ini_set('display_startup_errors', '1');
          error_reporting(E_ALL);
    ...
    // Fetch part numbers from the database
    $grn = $assetModel->select('grn')->groupBy('grn')->where('grn!=', '')->where('deleted_on', NULL)->findAll();
    
    // Return part numbers as JSON response
    return $this->response->setJSON($grn);
}
